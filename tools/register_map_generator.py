...
class Register(Node):
    ...
    Symbol:       str   | None = field(default=None, init=False)
    Scale:        float | None = field(default=None, init=False)
    ScaledUnits:  str   | None = field(default=None, init=False)
    DisplayName:  str   | None = field(default=None, init=False)
    ...
    def tags_to_attributes(self):
        ...
        if "symbol" in self.Tags:
            self.Symbol = self.Tags["symbol"].Value
        if "scale" in self.Tags:
            self.Scale = self.Tags["scale"].Value
        else:
            self.Scale = 1.0
            self.ScaledUnits = self.Units
        if "scaledunits" in self.Tags:
            self.ScaledUnits = self.Tags["scaledunits"].Value
        if "displayname" in self.Tags:
            self.DisplayName = self.Tags["displayname"].Value
    ...
    def calc_offset(self, next_offset, word_size=1, align=False):
        ...
        if align and next_offset % word_size != 0:
            ...
            print(SPACER*2 + f"{self.full_name()} padded from"
                  f" {SIZE_FROM_TYPE[self.Type]} to"
                  f" {next_offset-self.Offset} bytes for word alignment")
        print(SPACER*2 + f"{self.full_name()
                            } is {self.Offset} bytes offset from base")
        return next_offset
    ...
    def calc_size(self, word_size=1, align=False, block_size=None):
        ...
        if len(regs):
            ...
            if align and self.UsedSize % word_size != 0:
                self.AllocatedSize = (
                    self.UsedSize // word_size + 1) * word_size
            elif block_size:
            ...
            print(SPACER*2 + f"{self.full_name()
                                } allocates {self.AllocatedSize} bytes")
...
def build_map(filename="register_definitions.json",
    ...
    5) Generate a simplified and flattened view of the register as a JSON file
       for optimized use in a PC side parsing program.
    '''
    ...
    with open(filename, "br") as infile:
        schema_hash = file_digest(infile, "sha256").hexdigest()
    print(schema_hash)
    ...
    regs.WordSize = regs.Metadata.retrieve(
        "MemoryLayout").Tags["word_size"].Value
    ...
    # p = [[val.Value for key,val in group.Tags.items()] for group in regs.Metadata.retrieve("Plots").Groups]
    ...
    for sys in regs.Systems:
        ...
        for reg in sys.unroll(Register):
            next_offset = reg.calc_offset(
                next_offset, regs.WordSize, regs.Align)
    ...
    if not use_literals:
        for sys in regs.Systems:
            # Pass 3A - For each register generate a line of micropython code
            #           that defines a micropython.const() object.
            for reg in sys.unroll(Register):
                const_lines.append(reg.const_name(
                    f"OFFSET = const({reg.Offset})"))
                const_lines.append(reg.const_name(f"SIZE = const({reg.Size})"))
            const_lines.append("")
            # Pass 3B - For each register group generate a line of micropython
            #           code that defines a micropython.const() object.
            for group in sys.unroll(Group):
                const_lines.append(group.const_name(
                    f"OFFSET = const({group.Offset})"))
                const_lines.append(group.const_name(
                    f"ALLOC_SIZE = const({group.AllocatedSize})"))
                const_lines.append(group.const_name(
                    f"USED_SIZE = const({group.UsedSize})"))
            const_lines.append("")
        ...
        for group in reversed(sys.retrieve("Registers").unroll(Group)):
            ...
            for subgroup in group.Groups:
                if use_literals:
                    ...
                    relative_offset = f"{
                        subgroup.const_name("OFFSET")}-{group_offset}"
                ...
                if isinstance(subgroup, Group):
                    ...
                    if private_dicts:
                        s_line += "_"
                    s_line += f"{pascal_case_to_prefix(
                        sys.Name)+subgroup.Name[:3].upper()}),"
    ...
    for sys in regs.Systems:
        ...
        for group in sys.unroll(Group):
            ...
            if group.Alias is not None:
                struct_lines.append(
                    f"{pascal_case_to_prefix(sys.Name).lower()}_"
                    f"{group.Alias} = _array_view(buf, "
                    f"{group_offset}, {group_count}, ({group_count}, ))")

    # Pass 5 - For each register add a node to the metadata dictionary
    flat_metadata = {
        "schema_hash": schema_hash[:16],
        "frame_size":  regs.BlockSize * len(regs.Systems),
        "plots":       schema["Metadata"]["Plots"],
        "registers": []
    }

    for sys in regs.Systems:
        for reg in sys.retrieve("Registers").unroll(Register):
            flat_metadata["registers"].append({
                "name":         reg.Name,
                "path":         ".".join(reg.Path),
                "offset":       reg.Offset,
                "size":         reg.Size,
                ...
                "symbol":       reg.Symbol,
                "scale":        reg.Scale,
                "scaledunits":  reg.ScaledUnits,
                "displayname":  reg.DisplayName
            })

    with open(f"register_metadata_{schema_hash[:16]}.json", "w") as outfile:
        json.dump(flat_metadata, outfile, indent=2)

    # Coalesce all the lines and then write them to a file
    lines = []
    ...
    lines.append(f"schema_hash = 0x{schema_hash[:16]}")
    lines.append("")

    lines.append("def _array_view(buf, offset, count, shape):")
    lines.append("    # Allocate ndarray views once during setup."
                 " Runtime code should reuse the")
    lines.append("    # returned objects instead of calling"
                 " frombuffer in hot paths.")
    lines.append("    array = np.frombuffer(buf, dtype=np.float,"
